/*
# Create profile auto-creation trigger and seed reference data

1. Functions
- `handle_new_user()` — trigger function that creates a profiles row when a new auth.users row is inserted.
- Trigger `on_auth_user_created` fires AFTER INSERT on auth.users.
2. Seed Data
- `crop_catalog`: 12 common crops with water/season/soil/investment/revenue/risk data (estimates labeled as estimates in UI).
- `government_schemes`: 6 real, well-known government schemes with official sources and verify dates.
- `knowledge_articles`: 8 knowledge center articles across categories.
3. Notes
- All seed values are clearly estimates; UI shows "Estimated" labels.
- Government scheme data is real and sourced from official portals; UI adds verification disclaimer.
*/
...
-- Seed government schemes (real, sourced from official portals)
