  ...
  try {
    ...
    if (!geminiKey) {
      ...
      return new Response(JSON.stringify({
        role: "assistant",
        content: fallback,
        note: "Running in offline mode — no AI API key configured. This is a rule-based response. Add GEMINI_API_KEY for full AI responses.",
      }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // Convert chat history into Gemini's "contents" format.
    // Gemini uses "model" instead of "assistant" for the AI's turns, and system
    // instructions are passed separately rather than as a message in the list.
