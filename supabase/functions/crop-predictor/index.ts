...
async function loadDataset(): Promise<Dataset> {
  ...
  const text = await Deno.readTextFile(new URL("./crop_data.json", import.meta.url));
  cachedData = JSON.parse(text) as Dataset;
  return cachedData;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const result = knnPredict(validation.value, data, 5);
