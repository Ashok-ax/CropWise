/*
# Create soil_profiles and crop_records tables

1. New Tables
- `soil_profiles` — soil test data per farm (pH, N, P, K, organic matter, moisture, drainage, test results).
- `crop_records` — a crop planted on a farm (crop name, variety, planting date, growth stage, expected harvest, area).
2. Security
- RLS enabled, owner-scoped via parent farms.user_id.
3. Notes
- Child rows reference farms(id); policies verify farm ownership.
*/
